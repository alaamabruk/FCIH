...
<%-- 
    Document   : adminDeleteCours
    Created on : May 14, 2015, 4:34:43 PM
    Author     : Oscar
--%>
...
StudentAction op =new StudentAction();
   ...
   // std =(Student)session.getAttribute("currStudent");
   List<Course> lc = aop.retriveCourse();
...
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>adminDelete Cours</title>
    <h3> Admin Delete Course : </h3>
    </head>
    <br>
    <form action="<%= response.encodeURL("adminDeleteCourse")%>" method="post">
    <body>
    <table border="1">
        <% 
             for(Course s : lc) {
        %>
            ...
            <tr><td><input type="checkbox" name="co" value="<%= s.getName()%>"></td>
            <td> <%=s.getId() + s.getName()  %></td>
            </tr>
    ...
    <input type="reset" value="Reset"/><br><br>
             <a href="<%= response.encodeURL("adminPage.jsp")%>" ><h4>Back</h4></a>
    </form>
    </body>
</html>
